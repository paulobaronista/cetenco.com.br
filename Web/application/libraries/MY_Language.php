	...
	***************************************************/
	...
	function MY_Language()
	{
		...
		else	// URI without language -> redirect to default_uri
		{
			...
			// redirect
//			header("Location: " . $CFG->site_url($this->localized($this->default_uri.$RTR->uri->uri_string())), TRUE, 302);
			header("Location: " . $CFG->site_url($this->lang($this->languages[$this->default_lang()]) . $URI->uri_string()), TRUE, 302);
			exit;
		}
	}
	...
	function switch_uri($lang, $session_var = FALSE)
	{
		$CI =& get_instance();
		...
		// is there a Session variable to use?
		if ($session_var) {
			if ($this->has_language($uri)) {
				$CI->session->set_userdata($session_var,$this->lang());
			} else {
				$CI->session->set_userdata($session_var,$this->default_lang());
			}
		}
