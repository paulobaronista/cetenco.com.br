...
//					echo form_textarea(array('name'=>'embed', 'value'=>(isset($obra->embed) ? html_entity_decode($obra->embed) : '')));
		...
		<?php foreach($idiomas as $key => $value): ?>
			<div id="titulo_desc_<?=strtolower($value)?>">
				...
				echo form_input(Array('name' => 'titulo_'.$key, 'value' => set_value('titulo_'.$key, (isset($obra_desc[$key]->titulo) ? $obra_desc[$key]->titulo : '')), 'class' => 'titulo'));
				?>
				<div class="clr"></div>
				<?php
				echo form_label('Descrição', 'descricao');
				echo form_textarea(Array('name' => 'descricao_'.$key, 'id' => 'descricao', 'rows' => 4, 'cols' => '75', 'value' => set_value('descricao_'.$key, (isset($obra_desc[$key]->descricao) ? $obra_desc[$key]->descricao : ''))));
				?>
			</div>
		<?php endforeach;?>
		<?php echo form_close(); ?>
...
//					console.log(info);
		...
		$("div.caption a").live('click',function(e){
			e.preventDefault();
			if (typeof(e.detail) || e.detail == 1) {
				el = $(this);
				href = el.attr('href').split('/');
				file = href[href.length-1];
				action = href[href.length-2];
				if (action == 'excluir') {
					$.ajax({
						url: '<?=site_url('administracao/fotos/excluir')?>/'+file,
						type: 'POST',
						success: function(data, textStatus, jqXHR) {
							if (data == 'OK') {
								el.parents('li').remove();
								var ordem = $("#ulSort").sortable('serialize');
								$.post('<?=site_url('administracao/fotos/ordenar/'.$this->uri->segment(4))?>', ordem, function(info){
		//							console.log(info);
								});
							}
							return false;
					...
					$.ajax({
						url: '<?=site_url('administracao/fotos/destaque')?>/'+file,
						type: 'POST',
						data: 'idObra='+$("input[name=id]").val(),
						success: function(data, textStatus, jqXHR) {
							if (data == 'OK') {
								var text = 'Destaque';
								if ($("span.featured").length > 0) {
									var src = $("span.featured").parents('li').find('a').attr('href').split('/');
									var file = src[src.length-1];
									$('span.featured').replaceWith('<a class="regular" href="<?=site_url('administracao/fotos/destaque')?>/'+file+'">' + text + '</a>');
								}
								el.replaceWith('<span class="featured">' + text + '</span>');
							}
							return false;
						}
					});
