<div id="subHeader">
	...
	<div id="subHeaderRight">
		...
		<div id="btnNewFolder">
			<?=anchor(site_url('administracao/arquivos/criar_pasta'), img(array('src'=>base_url().'template/img/folder_files_64.png','width'=>'48px','height'=>'48px')) . '<br />Criar pasta', 'id=criar')?>
		</div>
		<?php if ($edit_folders) : ?>
		<div id="btnEditFolders">
			<?=anchor(site_url('administracao/arquivos/editar_pastas'), img(array('src'=>base_url().'template/img/folder_files_64.png','width'=>'48px','height'=>'48px')) . '<br />Editar Pastas', 'id=editar')?>
		</div>
		<?php endif; ?>
		<?php if (isset($pastas_vazias)) : ?>
		<div id="btnEmptyFolders">
			<?=anchor(site_url('administracao/arquivos/pastas_vazias'), img(array('src'=>base_url().'template/img/folder_files_64.png','width'=>'48px','height'=>'48px')) . '<br />Pastas Vazias', 'id=vazias')?>
		</div>
		<?php endif; ?>
	</div>
</div>
...
<div id="content">
	<?
	echo form_open(site_url('administracao/arquivos'));
	...
	echo form_submit(Array('name'=>'filtrar','value'=>'Filtrar','style'=>'width:auto;padding:2.5px'));
	echo form_close();
	?>
	<?php
		$oddCheck = 0;
