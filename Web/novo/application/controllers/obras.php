...
class Obras extends Controller {
	...
	function realizadas() {
		...
		if (!$this->uri->segment(4)) {
			...
			if (!array_key_exists($this->uri->segment(4), $data['categorias'])) {
				...
				$data['obra'] = $this->obras_model->lerDados($this->uri->segment(5))->row();
				...
				if ($data['fotos']->num_rows() > 0){
					$foto_cache = 'images/cache/'.$data['fotos']->row()->nome;

					if (!is_file($foto_cache)){
						$destaque_config = Array(
							'source_image'		=> 'images/' . $data['fotos']->row()->nome,
							'new_image'			=> 'images/cache/'.$data['fotos']->row()->nome,
							'maintain_ratio'	=> TRUE,
							'width'				=> 410,
							'height'			=> 302
						);

						$image_info = getimagesize('images/'.$data['fotos']->row()->nome);

						if ($image_info[0] > $image_info[1]) {
							$destaque_config['master_dim'] = 'height';
						} else {
							$destaque_config['master_dim'] = 'width';
						}
						...
						$destaque_config = array(
							'source_image'		=> 'images/cache/' . $data['fotos']->row()->nome,
	...
	//					die();
				...
				$data['page_title'] = $data['obra']->titulo;
...
//			}
			...
			if (!array_key_exists($this->uri->segment(4), $data['obras'])) {
				...
				$data['obra'] = $this->obras_model->lerDados($this->uri->segment(4))->row();
